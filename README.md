...
- 🚀 **Полная совместимость с Panel RIXXX & 3x-ui**:
  ...
  - Нативная поддержка **Федерации серверов RIXXX (v1.10+ / v1.11+)**: автоматический довыпуск («Deploy») ключей на все привязанные ноды и отзыв («Undeploy») по всей сетке.
...
- 💳 **Мульти-эквайринг и автоматизация оплат**:
  - **Lava.top**: Прием платежей через **СБП (Система быстрых платежей)**, QR-код, банковские карты РФ и зарубежные методы. Поддерживается гибридная отказоустойчивая схема подтверждения (Webhook + ручная проверка API).
...
4. **Подключение серверов (Хостов)**:
   - В разделе **Настройки** -> **Управление Хостами** добавьте ваш сервер:
     - **Для панели RIXXX:** обязательно укажите URL с данными Basic Auth: `https://логин:пароль@домен/секретный-путь/` (например, `https://admin:mypass@hub.mydomain.ru/32315d13040d6fea/`). В поле **ID входящего подключения** оставьте **1** (в RIXXX концепции инбаундов нет).
     - **Для панели 3x-ui:** укажите обычный URL (например, `http://IP:2053`), логин, пароль и ID вашего инбаунда VLESS.
...
### 1. Lava.top (СБП и Банковские карты) — Рекомендуется для физлиц

Платформа **Lava.top** идеально подходит для приема оплат в Telegram-шопах без открытия ИП или юрлица. Позволяет принимать платежи по **СБП (Система быстрых платежей)** прямо из мобильных приложений российских банков (Сбер, Т-Банк, ВТБ), а также по банковским картам.

#### ⚙️ Архитектурные и технические особенности реализации:
1. **Оффер с динамической ценой (Custom Price Invoices)**:
   - В Lava.top счета выставляются с привязкой к конкретному цифровому продукту. Чтобы бот мог автоматически списывать разные суммы для различных тарифов, продукт публикуется с признаком **«Цена по запросу через API»**. Бот передает `offerId` и конкретную сумму тарифа в рублях (`POST /api/v3/invoice`).
2. **Нативный режим СБП (PAY2ME / SBP)**:
   - Для мгновенной оплаты через СБП бот передает в API провайдер `paymentProvider: "PAY2ME"` и метод `paymentMethod: "SBP"`. Клиент получает прямую ссылку для оплаты в банковском приложении.
3. **Гибридная отказоустойчивая схема подтверждения (Webhook + API Poll)**:
   - Помимо входящего вебхука на эндпоинт `/lava-webhook`, бот выдает пользователю инлайн-кнопку **«🔄 Проверить оплату»**. При нажатии бот сам опрашивает API `GET /api/v1/invoices/{id}`. Если вебхук задерживается из-за очередей эквайера, ключ выдается моментально по прямому запросу.

#### 📋 Пошаговая настройка Lava.top:
1. Зарегистрируйтесь в личном кабинете на **[lava.top](https://app.lava.top)**.
2. Нажмите **«Создать» -> «Продукт» -> «Цифровой продукт»** (например, *«VPN Доступ»*).
   - В разделе цен включите тумблер **«Цена по запросу через API»** (динамическая цена).
   - ⚠️ **Важно:** Не создавайте продукт типа *«Подписка»* — у подписок в Лаве фиксированная цена, и при попытке бота передать сумму тарифа Лава вернет ошибку `Product is not dynamic price`.
   - **Как указать Offer ID:**
     - **Автоматически (Рекомендуется):** У товаров с динамической ценой в интерфейсе Лавы виден только UUID продукта из ссылки `https://app.lava.top/products/<UUID>`. Вы можете скопировать этот **Product ID** и вставить в поле **Lava.top Offer ID** в боте. Бот автоматически обратится к API Лавы (`GET /api/v2/products`) и сам найдёт скрытый `offerId`!
     - **Вручную через консоль:** Выполните на сервере команду:
       ```bash
       curl -s -H "X-Api-Key: ВАШ_LAVA_API_KEY" "https://gate.lava.top/api/v2/products?feedVisibility=ALL" | jq .
       ```
       В блоке вашего продукта найдите `offers -> [0] -> id` — это и есть `offerId`.
3. Перейдите в раздел **«Интеграции» -> «API»** и нажмите **«Настроить API key»**. Скопируйте созданный ключ (**Lava API Key**).
4. Нажмите **«Добавить Webhook»**:
   - **URL вебхука:** `https://your-domain.com/lava-webhook`
   - **Тип событий:** выберите `payment.success` (Успешная оплата) и `subscription.recurring.payment.success`.
   - **Тип аутентификации:** выберите **«API key вашего сервиса»** и введите произвольную секретную строку (например, надежный токен из 32 символов). Этот же ключ сохраните как **Lava.top Webhook Key** в панели бота.
5. В веб-панели управления ботом откройте **«Настройки» -> «Настройки Платежных Систем»**:
   - Укажите **Lava.top API Key**, **Lava.top Offer ID** и **Lava.top Webhook Key**.
   - Отметьте галочку *«Использовать СБП (PAY2ME/SBP) по умолчанию»*.
   - Сохраните настройки. В боте мгновенно появится кнопка **«⚡ СБП (Lava.top)»**.
...
### 4. Heleket & TonConnect
- Поддерживается мультивалютный крипто-эквайринг через **Heleket** (USDT, BTC) и нативное подключение некастодиальных TON-кошельков через **TonConnect** (Tonkeeper, MyTonWallet).
---

## ❓ FAQ и решение проблем

<details>
<summary><b>1. Где найти Inbound ID в панели RIXXX и 3x-ui?</b></summary>

- **В панели RIXXX:** концепции инбаундов **нет вообще**. Пользователь создается сразу под все протоколы (NaiveProxy, Mieru, Hysteria2). В форме бота в поле «ID входящего подключения» просто оставьте **`1`** (в коде этот параметр для RIXXX игнорируется).
- **В панели 3x-ui:** перейдите в раздел **"Подключения" (Inbounds)**. Номер в первой колонке таблицы и является `Inbound ID`.
</details>

<details>
<summary><b>2. Ошибка «Error logging in to host» или 401 при подключении панели RIXXX</b></summary>

Когда в панели RIXXX включен внешний доступ, Caddy защищает ее через **HTTP Basic Auth**. Если передать боту обычную ссылку, Caddy отсечет бота с ошибкой 401. 
**Решение:** укажите URL со встроенными учетными данными Caddy: `https://логин:пароль@домен/секретный-путь/`.
</details>

<details>
<summary><b>3. Ошибка 405 Method Not Allowed при открытии ссылки вебхука Lava</b></summary>

Если вы открыли `https://bot.domain.com/lava-webhook` в браузере и увидели `Method Not Allowed` — это значит, что порт и домен **работают правильно**. Браузер шлет GET-запрос, а вебхук принимает только POST с данными об оплате от серверов Lava.
</details>
<details>
<summary><b>4. Бот не реагирует на команду /give от саппорта</b></summary>
...
<details>
<summary><b>5. Как сбросить тестовый период пользователю?</b></summary>
...
<details>
<summary><b>6. Как перенести базу данных при обновлении сервера?</b></summary>
