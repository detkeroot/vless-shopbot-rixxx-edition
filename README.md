...
2. Создайте новый **«Цифровой продукт»** (например, *«VPN Доступ»*). При создании оффера выберите **«Цена по запросу через API»** (динамическая цена).
   - **Как правильно скопировать Offer ID:** в интерфейсе Lava нет кнопки «Скопировать Offer ID». Откройте ссылку на товар в браузере (в инкогнито), нажмите **«Купить»** (переход к экрану оплаты). Адресная строка изменится на `https://app.lava.top/products/UUID-товара/UUID-оффера?currency=RUB`. Скопируйте **ВТОРОЙ UUID** (который идет после слеша за UUID товара) — это и есть нужный **Offer ID**.
