...
### 3. Ошибки при создании счетов в Lava.top

#### А. Ошибка «Product is not dynamic price» (Status 400)
- **Причина:** Продукт в личном кабинете Lava создан с типом **«Подписка»** (Subscription). В Лаве подписки имеют фиксированную цену за период, а бот для гибких тарифов (1, 3, 6 месяцев) передает кастомную сумму в API (`amount`). Лава запрещает передавать произвольную сумму для фиксированных подписок.
- **Решение:** Создайте товар типа **«Цифровой продукт»** (Digital Product) и в блоке цен включите тумблер **«Цена по запросу через API»**.

#### Б. Ошибка «Product with offer id not found» (Status 404)
- **Причина:** В интерфейсе Лавы у товаров с динамической ценой скрыта кнопка «Купить», поэтому пользователь видит в URL только **Product ID** (`https://app.lava.top/products/<UUID>`). API Лавы (`/api/v3/invoice`) при этом строго требует внутренний **Offer ID**, из-за чего Лава отвечает 404.
- **Решение:**
  1. **Автоматически в боте:** Бот умеет **автоматически резолвить** Product ID в Offer ID. Если вставить UUID товара из адресной строки `https://app.lava.top/products/<UUID>` в поле `Offer ID` в боте, бот сам обратится к API Лавы (`GET /api/v2/products`) и подставит скрытый Offer ID.
  2. **Вручную через терминал:** Запустите команду:
     ```bash
     curl -s -H "X-Api-Key: ВАШ_LAVA_API_KEY" "https://gate.lava.top/api/v2/products?feedVisibility=ALL" | jq .
     ```
     В блоке вашего продукта найдите `offers -> [0] -> id`.
